...
export type CategoryKey = 'dev-tooling' | 'dev-onboarding' | 'dev-experiment' | 'protocol-infra' | 'dapp-idea';
export type EffortKey = 'Tiny' | 'Small' | 'Medium' | 'Large' | 'Extra Large';
export type SkillSetKey = 'Front End Development' | 'Back End Development' | 'Full Stack Development' | 'Smart Contract Development' | 'Protocol Development' | 'Education' | 'Content Creation';
export type ExecutionStatusKey = 'not-started' | 'in-discussion' | 'in-progress-open' | 'in-progress-closed' | 'completed' | 'abandoned';
export type LabelKey = 'OP Stack' | 'Chain Infrastructure' | 'Consumer dApps' | 'Smart Contracts' | 'Developer Tooling' | 'Documentation' | 'Attestations' | 'DeFi' | 'Gaming' | 'Governance' | 'NFTs' | 'Protocol' | 'DeSocial' | 'Wallets';
export type FilterKey = CategoryKey | EffortKey | ExecutionStatusKey | LabelKey | SkillSetKey;
